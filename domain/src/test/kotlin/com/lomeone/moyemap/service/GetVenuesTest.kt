...
class GetVenuesTest : BehaviorSpec({
    ...
    val venue1 = Venue("소셜파티1", VenueCategory.SOCIAL_PARTY, hongdaeLocation, 25000, imageUrl = "img1", description = "desc1", sourceUrl = "url1")
    val venue2 = Venue("혼술바1", VenueCategory.HONSOOL_BAR, gangnamLocation, 30000, imageUrl = "img2", description = "desc2", sourceUrl = "url2")
    val venue3 = Venue("소셜파티2", VenueCategory.SOCIAL_PARTY, gangnamLocation, 20000, imageUrl = "img3", description = "desc3", sourceUrl = "url3")

    Given("상태 필터 없이 조회할 때") {
        ...
        When("모든 Venue를 조회하면") {
            ...
            Then("PUBLISHED 상태의 Venue 목록이 반환된다") {
                ...
                verify { venueRepository.findByStatus(VenueStatus.PUBLISHED) }
            }
        }
    }

    Given("카테고리 필터가 주어졌을 때") {
        every { venueRepository.findByCategoryAndStatus(VenueCategory.SOCIAL_PARTY, VenueStatus.PUBLISHED) } returns listOf(venue1, venue3)

        When("특정 카테고리로 조회하면") {
            val result = getVenues(GetVenuesQuery(category = VenueCategory.SOCIAL_PARTY))

            Then("해당 카테고리의 Venue만 반환된다") {
                result shouldHaveSize 2
                result.all { it.category == VenueCategory.SOCIAL_PARTY } shouldBe true
                verify { venueRepository.findByCategoryAndStatus(VenueCategory.SOCIAL_PARTY, VenueStatus.PUBLISHED) }
            }
        }
    }
    ...
    Given("카테고리와 지역 필터가 함께 주어졌을 때") {
        ...
        When("카테고리와 지역으로 조회하면") {
            val result = getVenues(GetVenuesQuery(category = VenueCategory.SOCIAL_PARTY, region = "강남"))
    ...
    Given("가격 범위 필터가 주어졌을 때") {
        ...
        When("가격 범위로 조회하면") {
            ...
            Then("가격 범위 내의 Venue만 반환된다") {
                result shouldHaveSize 2
                result.all { it.price in 20000..25000 } shouldBe true
            }
        }
    }
})
