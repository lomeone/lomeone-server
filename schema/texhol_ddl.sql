...
CREATE TABLE games (
    ...
    PRIMARY KEY (game_id),
    UNIQUE KEY idx_games_store_id_name_u1 (store_id, name)
...
CREATE TABLE game_sessions (
    ...
    store_id BIGINT NOT NULL COMMENT '매장 ID (FK)',
    game_id BIGINT NOT NULL COMMENT '게임 ID (FK)',
    ...
    UNIQUE KEY idx_game_sessions_store_id_game_id_session_u1 (store_id, game_id, session),
    KEY idx_game_sessions_store_id_status (store_id, status),
    KEY idx_game_sessions_game_id (game_id)
...
-- Insert sample games
INSERT INTO games (store_id, name, schedule_type, day_of_week, description, created_at, updated_at) VALUES
(1, '데일리', 'DAILY', NULL, '매일 진행', NOW(), NOW()),
(1, '몬스터', 'DAILY', NULL, '매일 진행', NOW(), NOW()),
(1, '메인', 'WEEKLY', 6, '매주 토요일', NOW(), NOW());

-- Insert sample game sessions
INSERT INTO game_sessions (store_id, game_id, session, status, created_at, updated_at) VALUES
